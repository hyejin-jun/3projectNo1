...
public interface BoardAdminRepository extends JpaRepository<BoardAdmin, Long>, BoardAdminSearch {
    ...
    Page<BoardAdmin> findByBatitleContainingOrderByBanoDesc(String keyword, Pageable pageable); // 제목에 특정 키워드가 존재하는 게시글 찾기

    @Query("select b from BoardAdmin b where b.batitle like concat('%', : keyword, '%')")
        // 위에 거를 쿼리로 수정
