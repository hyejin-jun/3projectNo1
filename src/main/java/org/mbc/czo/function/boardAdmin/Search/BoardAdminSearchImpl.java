...
public class BoardAdminSearchImpl extends QuerydslRepositorySupport implements BoardAdminSearch { // 쿼리dsl레포지토리 상속받고 BoardAdminSearch 임폴트 받음
        ...
        public Page<BoardAdmin> search1(Pageable pageable) { // 위에 필드 클릭하면 자동 생성
            ...
            query.where(boardAdmin.batitle.contains("1"));
    ...
    public Page<BoardAdmin> searchAll(String[] types, String keyword, Pageable pageable) {  // 인터페이스에서 자동 생성
            ...
            if(types != null && types.length > 0 && keyword != null) {  // 검색 키워드와 조건이 있으면
                ...
                for(String type: types) {
                    switch (type){
                        case "t": // 제목 입력시
                            booleanBuilder.or(boardAdmin.batitle.contains(keyword));
                            break;
                        case"c": // 내용 입력시
                            booleanBuilder.or(boardAdmin.bacontent.contains(keyword));
                            break;
                        case "w":  // 작성자 입력시
                            booleanBuilder.or(boardAdmin.bawriter.contains(keyword));
                            break;
                    }  // 스위치 종료
                }  // 폴 종료
        ...
        query.where(boardAdmin.bano.gt(0L));
